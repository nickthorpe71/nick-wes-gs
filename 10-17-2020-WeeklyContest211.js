...
var maxLengthBetweenEqualCharacters = function(s) {
  ...
  for (let i = 0; i < array.length; i++) {
    if(!object[array[i]]){
      ...
      object[array[i]] = [object[array[i]][0], i];
    }
  }

  let greatestLength = 0;
  for(let letter in object){
    if (object[letter][1] - object[letter][0] > greatestLength) {
      greatestLength = object[letter][1] - object[letter][0];
    }
  }
  return greatestLength - 1; 
};

// console.log(maxLengthBetweenEqualCharacters('cabbac')); // 4

//2.
//if s.length % b === 0 then it can't rotate
//if s.length % b === odd then it can rotate to every digit
//if s.length % b === even then it can rotate to every other digit
...
//   if((s.length % b) % 2 === 0){
//     //can rotate to every other digit
//   }

//   if((s.length % b) % 2 === 1){
//     //can rotate to every digit
//   }
// };
...
//       if((ages[i] < ages[j] && scores[i] > scores[j]) || (ages[j] < ages[i] && scores[j] > scores[i])){
//         if(map[i]){
//           map[i] = [...map[i],j];
//         }
//         else {
...
//4.
function getDivisors(number,threshold){
  let divisors = [];
...
var areConnected = function(n, threshold, queries) {
  let map = {};
  for(let i = 1; i <= n; i++){
    map[i] = getDivisors(i,threshold);
  }

  let results = queries.map(nodePair => checkNodePair(nodePair,map));
  return results;
};

function checkNodePair(pair,map){
  for(let divisor in map[pair[0]]){
    if(map[pair[1]].includes(map[pair[0]][divisor])){
      return true;
    }
  }
  return false;
...
console.log(areConnected(6,2,[[1,4],[2,5],[3,6]])); //fft
console.log(areConnected(6,0,[[4,5],[3,4],[3,2],[2,6],[1,3]])); //ttttt
console.log(areConnected(5,1,[[4,5],[4,5],[3,2],[2,3],[3,4]])); //fffff
